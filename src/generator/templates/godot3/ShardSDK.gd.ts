...
var recording: Node = null
` : ''}${hasErrorTracking ? `
# =============================================================================
# Error Tracking Component
# =============================================================================

var errors: Node = null
` : ''}${hasMultiplayer ? `
# =============================================================================
# Multiplayer Component
# =============================================================================

var multiplayer_sdk: Node = null
...
${hasRecording ? `	# Initialize Recording SDK
	var RecordingSDKClass = load("res://addons/shard_sdk/recording/RecordingSDK.gd")
	recording = RecordingSDKClass.new()
	recording.name = "RecordingSDK"
	add_child(recording)
	_log("Recording SDK initialized")
` : ''}${hasErrorTracking ? `	# Initialize Error Tracking SDK
	var ErrorTrackingSDKClass = load("res://addons/shard_sdk/error_tracking/ErrorTrackingSDK.gd")
	errors = ErrorTrackingSDKClass.new()
	errors.name = "ErrorTrackingSDK"
	add_child(errors)
	_log("Error Tracking SDK initialized")
` : ''}${hasMultiplayer ? `	# Initialize Multiplayer SDK
	var ShardMultiplayerClass = load("res://addons/shard_sdk/multiplayer/ShardMultiplayer.gd")
	multiplayer_sdk = ShardMultiplayerClass.new()
...
func start_recording(event_id: String) -> String:
	if recording == null:
		...
		return ""
	return recording.start_recording(event_id)
...
func stop_recording(recording_id: String, delay_seconds: float = 0.0) -> void:
	if recording == null:
		push_error("ShardSDK: Recording feature not initialized")
		return
	recording.stop_recording(recording_id, delay_seconds)
