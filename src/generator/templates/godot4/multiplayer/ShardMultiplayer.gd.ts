...
/**
 * Godot 4.x ShardMultiplayer.gd template
 * Main multiplayer singleton - ported from Godot 3.x to use Godot 4 syntax
 */
export const shardMultiplayerTemplate: TemplateFunction = (context: TemplateContext): string => {
  return `extends Node
class_name ShardMultiplayer

## Shard Multiplayer SDK - Main Singleton
## Coordinates all child components and provides a unified API.
...
# Signals - Lobby
# =============================================================================
signal lobby_created(lobby_code: String, players: Dictionary)
signal lobby_joined(lobby_code: String, players: Dictionary)
signal lobby_left()
signal lobby_closed(reason: String)
signal lobby_error(code: String, message: String)
signal player_joined(peer_id: int, username: String)
signal player_left(peer_id: int, username: String, new_host_peer_id: int)
...
signal rate_limited()
signal prediction_corrected(server_pos: Vector2, predicted_pos: Vector2, correction_amount: float)

# =============================================================================
# Child Components
# =============================================================================
var _connection: ShardConnection
var _lobby: ShardLobby
var _match: ShardMatch
var _input: ShardInput
var _state: ShardState
var _prediction: ShardPrediction
...
func _initialize_components() -> void:
	_connection = ShardConnection.new()
	_lobby = ShardLobby.new()
	_match = ShardMatch.new()
	_input = ShardInput.new()
	_state = ShardState.new()
	_prediction = ShardPrediction.new()
...
func _wire_signals() -> void:
	...
	_connection.authenticated.connect(_on_connection_authenticated)
	_connection.authentication_failed.connect(_on_connection_authentication_failed)
	_connection.reconnected.connect(_on_connection_reconnected)
	_connection.reconnection_failed.connect(_on_connection_reconnection_failed)
	_connection.connection_lost.connect(_on_connection_lost)
	_connection.rate_limited.connect(_on_connection_rate_limited)
	_connection.error_received.connect(_on_connection_error)
	_connection.message_received.connect(_on_message_received)
	_connection.latency_updated.connect(_on_latency_updated)
	
	# Lobby signals
	_lobby.lobby_created.connect(_on_lobby_created)
	_lobby.lobby_joined.connect(_on_lobby_joined)
	_lobby.lobby_left.connect(_on_lobby_left)
	...
	# Match signals
	_match.countdown.connect(_on_countdown)
	...
	# State signals
	_state.state_received.connect(_on_state_received)
	
	# Prediction signals
	_prediction.prediction_corrected.connect(_on_prediction_corrected)
...
func _on_message_received(message: Dictionary) -> void:
	...
	if _match.handle_message(message):
		if msg_type == ShardProtocol.MSG_ROUND_START:
			_connection.set_in_match(true)
		elif msg_type == ShardProtocol.MSG_MATCH_OVER:
			_connection.set_in_match(false)
		return
	
	if msg_type == ShardProtocol.MSG_STATE_SNAPSHOT:
...
func _handle_game_event(message: Dictionary) -> void:
	...
	match msg_type:
		ShardProtocol.MSG_PLAYER_DAMAGED:
			...
			_log("Player damaged: attacker=%d victim=%d damage=%d health=%d" % [attacker, victim, damage, health])
			player_damaged.emit(attacker, victim, damage, health)
		
		ShardProtocol.MSG_PLAYER_ELIMINATED:
			var peer_id = int(message.get("peer_id", -1))
			var killer = int(message.get("killer_peer_id", -1))
			_log("Player eliminated: peer_id=%d killer=%d" % [peer_id, killer])
			player_eliminated.emit(peer_id, killer)
		
		ShardProtocol.MSG_PLAYER_DISCONNECTED:
			var peer_id = int(message.get("peer_id", -1))
			var username = str(message.get("username", ""))
			_log("Player disconnected: %s (peer_id=%d)" % [username, peer_id])
			player_disconnected.emit(peer_id, username)
		
		ShardProtocol.MSG_PLAYER_RECONNECTED:
			var peer_id = int(message.get("peer_id", -1))
			var username = str(message.get("username", ""))
			_log("Player reconnected: %s (peer_id=%d)" % [username, peer_id])
			player_reconnected.emit(peer_id, username)
		
		ShardProtocol.MSG_PLAYER_TIMEOUT:
			var peer_id = int(message.get("peer_id", -1))
			var username = str(message.get("username", ""))
			_log("Player timed out: %s (peer_id=%d)" % [username, peer_id])
		...
		ShardProtocol.MSG_WEAPON_DROPPED:
			...
			_log("Weapon dropped: peer_id=%d weapon=%s pos=%s" % [peer_id, weapon_type, str(pos)])
			weapon_dropped.emit(peer_id, weapon_type, pos)
		
		ShardProtocol.MSG_ERROR:
...
func _handle_error(message: Dictionary) -> void:
	...
	if not ShardProtocol.is_lobby_error(code):
...
func _on_connection_authenticated(user_id: String, username: String) -> void:
	...
	authenticated.emit(user_id, username)
...
func _on_lobby_created(lobby_code: String, players: Dictionary) -> void:
	...
	lobby_created.emit(lobby_code, players)
...
func _on_lobby_joined(lobby_code: String, players: Dictionary) -> void:
	...
	_sync_peer_id_to_prediction()
	lobby_joined.emit(lobby_code, players)
...
func _on_player_joined(peer_id: int, username: String) -> void:
	...
	player_joined.emit(peer_id, username)
...
func _on_player_left(peer_id: int, username: String, new_host_peer_id: int) -> void:
	...
	player_left.emit(peer_id, username, new_host_peer_id)
...
func _on_match_ended(winner_peer_id: int, winner_username: String, final_scores: Array) -> void:
	...
	_connection.set_in_match(false)
	match_ended.emit(winner_peer_id, winner_username, final_scores)
...
func _on_prediction_corrected(server_pos: Vector2, predicted_pos: Vector2, correction_amount: float) -> void:
	...
	prediction_corrected.emit(server_pos, predicted_pos, correction_amount)
