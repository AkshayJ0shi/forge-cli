...
/**
 * Godot 4.x ShardPrediction.gd template
...
export const shardPredictionTemplate: TemplateFunction = (context: TemplateContext): string => {
  return `class_name ShardPrediction
extends RefCounted
...
func reconcile(state: ShardState, input_handler: ShardInput) -> void:
	...
	if diff > CORRECTION_THRESHOLD:
		...
		prediction_corrected.emit(server_pos, old_predicted, diff)
	
	input_handler.clear_inputs_before_tick(last_processed)


func _find_my_player(players: Array) -> Variant:
...
func initialize_from_server_state(state: ShardState) -> void:
	if _my_peer_id < 0:
		return
