...
/**
 * Godot 4.x ShardLobby.gd template
...
func join_lobby(connection: ShardConnection, lobby_code: String) -> void:
	if connection == null or not connection.is_authenticated():
	...
	if lobby_code.is_empty():
		...
		lobby_error.emit(ShardProtocol.ERR_INVALID_LOBBY_CODE, "Lobby code cannot be empty")
		return
	
	var message = {
		"type": ShardProtocol.MSG_JOIN_LOBBY,
		"timestamp": Time.get_ticks_msec(),
...
func leave_lobby(connection: ShardConnection) -> void:
	if connection == null or not connection.is_authenticated():
	...
	var message = {
		"type": ShardProtocol.MSG_LEAVE_LOBBY,
		"timestamp": Time.get_ticks_msec()
	}
	connection.send_message(message)
...
func handle_message(message: Dictionary) -> bool:
	...
	match message.type:
		ShardProtocol.MSG_LOBBY_CREATED:
			_handle_lobby_created(message)
			return true
		
		ShardProtocol.MSG_LOBBY_JOINED:
			_handle_lobby_joined(message)
			return true
		
		ShardProtocol.MSG_PLAYER_JOINED:
			_handle_player_joined(message)
			return true
		
		ShardProtocol.MSG_PLAYER_LEFT:
			_handle_player_left(message)
			return true
		
		ShardProtocol.MSG_LOBBY_CLOSED:
...
func _handle_lobby_created(message: Dictionary) -> void:
	...
	lobby_created.emit(_lobby_code, _players.duplicate())
...
func _handle_lobby_joined(message: Dictionary) -> void:
	...
	lobby_joined.emit(_lobby_code, _players.duplicate())
...
func _handle_error(message: Dictionary) -> bool:
	...
	if ShardProtocol.is_lobby_error(code):
		_log("Lobby error: %s - %s" % [code, error_message])
		lobby_error.emit(code, error_message)
		return true
...
func _parse_players(players_array: Variant) -> void:
	_players.clear()
	_host_peer_id = -1
...
func is_in_lobby() -> bool:
	return not _lobby_code.is_empty()
